...
public class FeedbackDatabase {
	...
	public long createEntry(String userid, String event,String eventid,String rating, String comment, String check, int choice){
		ContentValues cv = new ContentValues();
		cv.put(KEY_EVENT, event);
		cv.put(KEY_USERID, userid);
		cv.put(KEY_EVENTID, eventid);
        cv.put(KEY_CHOICE,choice);
        cv.put(KEY_RATING,rating);
        cv.put(KEY_COMMENT,comment);
       cv.put(KEY_CHECK,check);
	...
	public String[][] getData(){
		int count=0;
		int i=0;
		String[] columns = new String[]{KEY_ROWID,KEY_USERID,KEY_EVENT,KEY_EVENTID,KEY_RATING,KEY_COMMENT,KEY_CHECK,KEY_CHOICE};
        ...
        int check = c.getColumnIndex(KEY_CHECK);
        int comment = c.getColumnIndex(KEY_COMMENT);
        int rating = c.getColumnIndex(KEY_RATING);
		...
		String[][] data = new String[7][count];
		for(c.moveToFirst(); !c.isAfterLast(); c.moveToNext()){
				data[0][i] = c.getString(row);
				data[1][i] = c.getString(userid);
				data[2][i] = c.getString(event);
				/*if(c.getInt(rating)>0)
					data[3][i] = "sent";
				else if(c.getInt(sent)==0)
					data[3][i] = "not sent";
				else */
            if(c.getString(rating).equals("-1"))
					data[3][i] = "RATING";
            else
                data[3][i] =c.getString(rating);
                data[4][i]=c.getString(comment);
            data[5][i]=c.getString(check);
            if(c.getInt(choice)==-1)
                data[6][i]="CHOICE";
            else
                data[6][i]=c.getString(choice);
				i++;
		}
		return data;
	}
